...
from catconnect.forms import RegisterForm, LoginForm, CatAdoptForm, CancelAdoptForm, AddForm
...
def add_page():
    form = AddForm()
    if form.validate_on_submit():
        item_to_create = Item(name=form.cat_name.data,
                              age=form.cat_age.data,
                              breed=form.cat_breed.data,
                              description=form.cat_description.data)
        db.session.add(item_to_create)
        db.session.commit()
        flash (f"Congratulations! You have added {item_to_create.name}.", category='success')
        return redirect(url_for('adoption_page'))
    if form.errors != {}:
        for err_msg in form.errors.values():
            flash(f'There was an error in adding a cat: {err_msg}', category='danger')
